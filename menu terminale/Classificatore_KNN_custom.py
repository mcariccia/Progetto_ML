...
def tuning(training_x, test_x, training_y, test_y):
...
def tuning_result (test_y, pred_y, best_k, best_dist):
...
def knn(X, Y):
    ...
    training_x, test_x, training_y, test_y = train_test_split(X, Y, random_state=0,test_size=0.25,stratify=Y)
    
    training_x, test_x, training_y, test_y = training_x.to_numpy(), test_x.to_numpy(), training_y.to_numpy(), test_y.to_numpy()
    
    best_clf=KNNCustom(k= 3,dist='distanza_manhattan')
    best_clf.fit(training_x,training_y)
    pred_y=best_clf.predict(test_x)
    
    eps = np.finfo(float).eps
    cms = multilabel_confusion_matrix(test_y, pred_y, labels=np.unique(test_y)) #calcoliamo matrici di confusione per ogni classe
    TPR,TNR,FPR,FNR,p,r,f1=compute_performances_multiclass(cms)  #calcoliamo le medie delle statistiche
    
    print(f'L\'accuratezza è del {round(compute_accuracy(pred_y, test_y),2)}')
    
    print('\nPiù nello specifico in media si sono ottenute sulle classi queste performance:')
    print(f'TPR medio:{round(TPR,2)}\nTNR medio:{round(TNR,2)}\nFPR medio:{round(FPR,2)}\nFNR medio:{round(FNR,2)}\nprecision medio:{round(p,2)}\nrecall medio:{round(r,2)}\nf1 score medio:{round(f1,2)}\n')
